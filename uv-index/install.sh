#!/bin/bash
# =============================================================================
#  UV Index — установщик плагина для Home Screens
...
main() {
  ...
  check_root
  resolve_paths
  echo -e "${BLUE}UV Index Plugin — установка (пользователь: $REAL_USER, HS: $HS_DIR)${NC}"
  install_plugin
  restart_hs
  echo ""
  echo -e "${GREEN}═══════════════════════════════════════${NC}"
  echo -e "${GREEN}  UV Index v${PLUGIN_VERSION} установлен${NC}"
  echo -e "${GREEN}═══════════════════════════════════════${NC}"
  echo "  Откройте редактор → палитра → «Weather & Environment» → «UV Index»"
  echo "  Настройте широту/долготу вашего местоположения."
  echo ""
}
